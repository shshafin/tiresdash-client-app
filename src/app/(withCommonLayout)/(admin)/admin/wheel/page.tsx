...
const Page = () => {
  ...
  const { mutate: handleDeleteWheel, isPending: deleteWheelPending } = useDeleteWheel({
    onSuccess: () => {
  ...
  const totalItems = Wheels?.data?.length ?? 0;
  ...
  const paginatedWheels = useMemo(() => {
    if (!Wheels?.data) return Wheels;
    const start = (currentPage - 1) * pageSize;
    const end = start + pageSize;
    return { ...Wheels, data: Wheels.data.slice(start, end) };
  }, [Wheels, currentPage, pageSize]);

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-md md:text-3xl font-semibold text-gray-900 dark:text-white">Wheels</h1>
      ...
      <ImportCSVWheelsModal isOpen={isOpen} onOpenChange={onOpenChange} />
    </div>
  );
};
...
const DeleteWheelModal = ({ isOpen, onOpenChange, handleDeleteWheel, deleteWheelPending }: any) => {
  return (
          ...
          <>
            ...
            <ModalBody>
              <p className="text-sm text-red-500">
                ⚠️ Are you sure you want to delete this Wheel? This action cannot be undone.
              </p>
            </ModalBody>
            ...
            <ModalFooter className="flex justify-end gap-2">
              ...
              <Button color="danger" onPress={handleDeleteWheel} disabled={deleteWheelPending} className="rounded">
