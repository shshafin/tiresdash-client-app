...
function transformData(tireArray: any, brand: any) {
  return (
    tireArray?.slice(0, 4).map((tire: any) => ({
      _id: tire?._id,
      brand,
...
const TireDealsSection = ({ brand }: any) => {
  ...
  if (isLoading) {
    return (
      <div className="flex h-[40vh] items-center justify-center">
        ...
        <span className="ml-2 font-medium">Fetching trending deals...</span>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-[40vh] flex-col items-center justify-center gap-4 text-center px-4">
        <p className="text-lg font-semibold text-red-500">
          Unable to load {brand?.name} deals
        </p>
        <Button
          variant="flat"
          color="danger"
  ...
  return (
    <div className="w-full py-10 md:py-20 bg-white dark:bg-black/20">
      <div className="container mx-auto px-4">
        {/* Header Section */}
        <div className="mb-8 md:mb-12 text-center md:text-left">
          <h2 className="text-2xl md:text-4xl font-black mb-3 text-gray-900 dark:text-white">
            TRENDING{" "}
            <span className="text-red-600 uppercase">{brand?.name}</span> DEALS
          </h2>
        ...
        {tireData && tireData.length > 0 ? (
            ...
            <div className="flex justify-center mt-4">
              <Link
                ...
                <Button
                  ...
                  EXPLORE ALL {brand?.name} SAVINGS
                </Button>
              </Link>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-gray-100 rounded-3xl bg-gray-50/50">
            <ShoppingCart className="mb-4 w-12 h-12 text-gray-300" />
            <p className="text-gray-500 text-lg font-medium px-6">
              Currently, there are no active tire deals for{" "}
