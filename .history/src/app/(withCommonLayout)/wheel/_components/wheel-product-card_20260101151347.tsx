...
const WheelCard = ({ wheel }: { wheel: any }) => {
  ...
  const { mutate: handleAddItemToCart, isPending } = useAddItemToCart({
    onSuccess: () => {
      ...
      router.push("/cart"); // ✅ মিশন ৩: Buy Now ডিরেক্ট কার্ট
    },
    userId: user?._id,
  });
  ...
  return (
    <div className="relative mt-20 mb-2 group h-full">
      {/* --- ১. সিগনেচার ওভারহ্যাংগিং হুইল ইমেজ (Racing Look) --- */}
      <div
        onClick={() => router.push(`/wheel/${wheel?._id}`)}
        className="absolute -top-20 -left-6 z-30 w-48 h-48 sm:w-56 sm:h-56 cursor-pointer transition-transform duration-500 group-hover:scale-110 group-hover:rotate-12">
        <img
          src={`${process.env.NEXT_PUBLIC_BASE_URL}${wheel.images?.[0]}`}
          alt={wheel.name}
          className="object-contain w-full h-full drop-shadow-[0_25px_30px_rgba(0,0,0,0.45)]"
        />
      </div>
      ...
      <div className="bg-white dark:bg-[#1a1d23] rounded-[48px] p-5 pt-24 border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col transition-all duration-500 hover:shadow-2xl hover:-translate-y-2">
        ...
        <div className="absolute top-8 right-8 flex flex-col gap-1.5 items-end">
          {wheel.finish && (
            <div className="bg-orange-50 dark:bg-orange-950/30 px-3 py-1 rounded-xl flex items-center gap-2 border border-orange-100/50 shadow-sm">
              ...
              />
              <span className="text-[9px] font-black uppercase text-orange-600 dark:text-orange-400 tracking-tighter">
                {wheel.finish}
              </span>
            </div>
          )}
          {wheel.boltPattern && (
            <div className="bg-blue-50 dark:bg-blue-950/30 px-3 py-1 rounded-xl flex items-center gap-2 border border-blue-100/50 shadow-sm">
              <Gauge
                size={12}
        ...
        <div
          ...
          <h3 className="relative w-fit text-2xl font-black text-gray-900 dark:text-white uppercase italic tracking-tighter leading-tight group-hover:text-blue-700 transition-colors">
            {wheel?.name}
            <span className="absolute left-0 -bottom-1 w-0 h-1 bg-blue-700 transition-all duration-300 group-hover:w-full"></span>
          </h3>
        </div>

        {/* --- ৪. কুইক প্যাক ডিলস (Pair/Set) --- */}
          ...
          <div className="flex flex-col shrink-0">
            {wheel?.discountPrice && wheel?.discountPrice < wheel?.price ? (
              <>
                ...
                <span className="text-3xl font-black text-gray-900 dark:text-white tracking-tighter italic leading-none">
                  ${wheel?.discountPrice}
                </span>
              </>
            ) : (
