...
const SingleOrderPage = () => {
  ...
  const handleExportPDF = async () => {
    try {
        ...
        doc.text(`${order.user.firstName} ${order.user.lastName}`.toUpperCase(), 20, y + 7);
        doc.setFont("helvetica", "normal");
        doc.setFontSize(9);
        ...
        doc.setTextColor(15, 23, 42);
        doc.setFont("helvetica", "normal");
        doc.text(`NO:      #${order._id.slice(-8).toUpperCase()}`, pageWidth - 80, y + 7);
        doc.text(`DATE:    ${new Date(order.createdAt).toLocaleDateString()}`, pageWidth - 80, y + 12);
        ...
        doc.text(`${addr.street}, ${addr.city}, ${addr.state} ${addr.postalCode}, ${addr.country}`, 25, y + 16);
        ...
        order.items.forEach((item, index) => {
            ...
            if (item.installationFee || item.addonServices?.length) {
                ...
                if (item.installationFee && item.installationFee > 0) {
                    doc.text(`+ ${item.installationService} (Unit: $${item.installationFee})`, 28, y);
        ...
        doc.setTextColor(15, 23, 42);
        doc.text(`$${services.toFixed(2)}`, pageWidth - 25, y, { align: "right" });
        ...
        doc.text(`$${order.totalPrice.toFixed(2)}`, pageWidth - 25, y + 8, { align: "right" });

        // --- ৭. প্রিমিয়াম ফুটার ---
        doc.setTextColor(148, 163, 184);
        doc.setFontSize(7);
        doc.setFont("helvetica", "normal");
        doc.text("TERMS: THIS INVOICE IS VALID FOR PREMIUM WARRANTY CLAIMS. KEEP IT SECURE.", pageWidth / 2, 280, { align: "center" });
        doc.text("THANK YOU FOR CHOOSING TIRESDASH FOR YOUR AUTOMOTIVE EXCELLENCE.", pageWidth / 2, 285, { align: "center" });
