...
const handleExportPDF = async () => {
    try {
        toast("Preparing your professional invoice...");

        const doc = new jsPDF();
        const pageWidth = doc.internal.pageSize.width;
        
        // --- ১. হেডার ডিজাইন (Brand Header) ---
        ...
        doc.text("TIRESDASH", 20, 25);
        
        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.text("Premium Automotive Services", 20, 32);
        doc.text("Website: tiresdash.com", 20, 37);

        doc.setFontSize(28);
        doc.text("INVOICE", pageWidth - 20, 30, { align: "right" });

        // --- ২. ইনভয়েস ইনফো (Order Summary) ---
        ...
        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);
        doc.text(`${order.user.firstName} ${order.user.lastName}`, 20, y + 7);
        doc.text(`Email: ${order.user.email}`, 20, y + 12);
        doc.text(`Phone: ${order.user.phone}`, 20, y + 17);

        // অর্ডার সেকশন (ডান পাশে)
        doc.setFontSize(12);
        doc.setFont("helvetica", "bold");
        doc.text("ORDER INFO:", pageWidth - 80, y);
        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);
        doc.text(`Invoice No: INV-${order._id.slice(-6).toUpperCase()}`, pageWidth - 80, y + 7);
        doc.text(`Order Date: ${new Date(order.createdAt).toLocaleDateString()}`, pageWidth - 80, y + 12);
        doc.text(`Status: ${order.status.toUpperCase()}`, pageWidth - 80, y + 17);
        ...
        doc.text(`${address.street}, ${address.city}`, 20, y + 7);
        doc.text(`${address.state}, ${address.postalCode}, ${address.country}`, 20, y + 12);
        ...
        doc.text("Unit Price", 150, y + 7, { align: "center" });
        doc.text("Total", pageWidth - 25, y + 7, { align: "right" });

        // --- ৪. আইটেম লুপ (Items & Services) ---
        y += 18;
        doc.setFont("helvetica", "normal");
        ...
        order.items.forEach((item) => {
            ...
            doc.text(`$${(item.price * item.quantity).toFixed(2)}`, pageWidth - 25, y, { align: "right" });
            y += 7;

            // ইনস্টলেশন ফি (যদি থাকে)
            if (item.installationFee && item.installationFee > 0) {
                doc.setFontSize(9);
                doc.setTextColor(100);
                doc.text(`   + ${item.installationService}`, 25, y);
                doc.text(`$${(item.installationFee * item.quantity).toFixed(2)}`, pageWidth - 25, y, { align: "right" });
                y += 5;
            }

            // অ্যাডন সার্ভিসেস (যদি থাকে)
            item.addonServices?.forEach((addon) => {
                doc.setFontSize(9);
                doc.setTextColor(100);
                doc.text(`   + ${addon.name}`, 25, y);
                doc.text(`$${(addon.price * item.quantity).toFixed(2)}`, pageWidth - 25, y, { align: "right" });
                y += 5;
            });

            doc.setTextColor(0);
            doc.setFontSize(10);
            y += 5; // আইটেমগুলোর মাঝে গ্যাপ
        ...
        doc.text(`$${order.totalPrice.toFixed(2)}`, pageWidth - 25, y, { align: "right" });

        // --- ৬. ফুটার ---
        doc.setTextColor(150);
        doc.setFontSize(8);
        doc.setFont("helvetica", "normal");
        doc.text("Thank you for choosing Tiresdash. This is a computer generated invoice.", pageWidth / 2, 285, { align: "center" });
        ...
        console.error(error);
        toast.error("Could not generate professional PDF.");
    }
};
