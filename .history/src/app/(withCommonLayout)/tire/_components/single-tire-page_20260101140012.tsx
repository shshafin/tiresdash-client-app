...
<div className="space-y-6">
  <div className="flex flex-wrap gap-3">
    {/* Pair (2) Button - Original Rose Gradient */}
    <button
      onClick={() => {
        setTireSet(2);
        setQuantity(1);
      ...
      className={`flex-1 min-w-[140px] py-5 rounded-[24px] transition-all border-2 flex flex-col items-center gap-1 shadow-lg ${
        tireSet === 2 
          ? "bg-gradient-to-br from-rose-600 to-rose-900 border-rose-500 text-white scale-[1.02]" 
          : "bg-gradient-to-br from-rose-600 to-rose-900 border-rose-500 text-gray-50 opacity-60 hover:opacity-100"
      }`}
    >
      <span className="font-black text-sm uppercase italic">Buy Pair (2 PC)</span>
      <span className="text-[9px] font-bold uppercase tracking-tighter opacity-80">Safety & Balance</span>
    </button>

    {/* Full Set (4) Button - Original Emerald Gradient */}
    <button
      onClick={() => {
        setTireSet(4);
        setQuantity(1);
      ...
      className={`flex-1 min-w-[140px] py-5 rounded-[24px] transition-all border-2 flex flex-col items-center gap-1 shadow-lg ${
        tireSet === 4 
          ? "bg-gradient-to-br from-emerald-600 to-emerald-900 border-emerald-500 text-white scale-[1.02]" 
          : "bg-gradient-to-br from-emerald-600 to-emerald-900 border-emerald-500 text-gray-50 opacity-60 hover:opacity-100"
      }`}
    >
      <span className="font-black text-sm uppercase italic">Full Set (4 PC)</span>
      <span className="text-[9px] font-bold uppercase tracking-tighter opacity-80">Ultimate Performance</span>
    </button>

    {/* ✅ মিশন ৪: Single Unit Toggle (Only shows when 2 or 4 is selected) */}
    {(tireSet === 2 || tireSet === 4) && (
      <button
        onClick={() => {
          setTireSet(1);
      ...
      >
        <div className="flex items-center justify-center gap-2">
           <Zap size={14} /> RETURN TO SINGLE UNIT (1 PC)
        </div>
      </button>
    )}
  </div>

  {/* Quantity & Buy Now Action Section */}
  <div className="flex flex-col md:flex-row gap-4">
    ...
    <div className="flex items-center gap-4 bg-gray-50 dark:bg-[#1a1d23] p-2 rounded-[24px] w-full md:w-fit border border-gray-100 dark:border-gray-800 justify-between px-6">
      ...
      <button
        onClick={() => setQuantity(Math.min(tire?.stockQuantity || 100, quantity + 1))}
        className="h-12 w-12 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-gray-800 transition-all shadow-sm text-gray-900 dark:text-white"
      >
        <Plus size={18} />
      </button>
    ...
    {/* ✅ মিশন ৩: Buy Now Button - Direct to Cart */}
    <Button
      ...
      onPress={() =>
        addToCart({
          productType: "tire",
          productId: tire._id,
          quantity: finalQuantity, // Calculates (quantity * tireSet)
        })
      }
      className="w-full md:flex-1 h-20 bg-gradient-to-r from-blue-800 via-blue-700 to-blue-600 hover:from-blue-900 text-white rounded-[24px] text-lg font-black uppercase tracking-widest shadow-xl shadow-blue-500/30 active:scale-95 transition-all group"
    >
      <div className="flex items-center gap-3">
        <ShoppingCart size={22} className="group-hover:rotate-[-12deg] transition-transform" />
        BUY NOW 
