...
const SingleTirePage = ({ params }: { params: { id: string } }) => {
  ...
  const [tireSet, setTireSet] = useState(2); // Default to Pair (Marketing Logic)
  const [showReviewForm, setShowReviewForm] = useState(false);
  const [reviewForm, setReviewForm] = useState({ rating: 5, comment: "" });

  // ✅ ১. Buy Now Logic: এড করার পর সরাসরি কার্ট পেজে নিয়ে যাবে
  const { mutate: addToCart, isPending: addingToCart } = useAddItemToCart({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["GET_CART"] });
      toast.success("Speeding to your cart...");
      router.push("/cart"); // সরাসরি রিডাইরেক্ট
    },
    userId: user?._id,
  });
  ...
  // ✅ ২. Review Create Logic: invalidateQueries দিয়ে ইনস্ট্যান্ট আপডেট ফিক্স
  ...
  return (
    <div className="bg-white dark:bg-[#0f1115] min-h-screen">
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-[10px] md:text-sm font-black uppercase tracking-widest text-gray-400 hover:text-orange-600 mb-6 transition-colors">
            ...
            <div className="space-y-6">
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 py-5 rounded-[24px] transition-all border-2 flex flex-col items-center gap-1 shadow-lg ${tireSet === 2 ? "bg-gradient-to-br from-rose-600 to-rose-900 border-rose-500 text-white scale-[1.02]" : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-400 opacity-60"}`}>
                ...
                <button
                  ...
                  className={`flex-1 py-5 rounded-[24px] transition-all border-2 flex flex-col items-center gap-1 shadow-lg ${tireSet === 4 ? "bg-gradient-to-br from-emerald-600 to-emerald-900 border-emerald-500 text-white scale-[1.02]" : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-400 opacity-60"}`}>
                  <span className="font-black text-sm uppercase italic">
                    Full Set (4 PC)
                  </span>
                  <span className="text-[9px] font-bold uppercase tracking-tighter opacity-80">
        ...
        {/* ✅ ৩. Full Description & Full Performance Specs Restored */}
                    ...
                    <Card className="rounded-[32px] p-2 bg-orange-50 dark:bg-orange-950/10 border-2 border-dashed border-orange-200">
                      <CardBody className="p-6">
                        ...
                        <textarea
                          ...
                          placeholder="Describe the grip, noise, and durability..."
                        />
                        <Button
                          onPress={() =>
                            addReview({
