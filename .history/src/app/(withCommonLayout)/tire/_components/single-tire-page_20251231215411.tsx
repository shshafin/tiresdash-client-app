...
const SingleTirePage = ({ params }: { params: { id: string } }) => {
  ...
  const hasAlreadyReviewed = reviews.some((r: any) => r.user?._id === user?._id);
  ...
  if (isLoading)
    return (
      <div className="flex h-screen items-center justify-center bg-white">
        <Zap className="animate-spin text-orange-600" size={48} />
      </div>
    );
    ...
    <div className="bg-white dark:bg-[#0f1115] min-h-screen">
      <div className="container mx-auto px-4 py-4 md:py-6 max-w-7xl">
                ...
                {[1, 2, 4].map((s) => (
                  ...
                  >
                    <span className="uppercase">
                      {s === 1 ? "Single Unit" : s === 2 ? "Safety Pair" : "Road Set"}
                    </span>
                    {s > 1 && (
                      <span className={`text-[8px] px-2 rounded-full font-bold ${tireSet === s ? "bg-white text-orange-600" : "bg-orange-100 text-orange-600"}`}>
                        BUNDLE SAVINGS
                      </span>
                    )}
                  </button>
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                >
                  <span className="flex items-center gap-2">
                    DEPLOY NOW <ChevronRight size={20} className="animate-pulse" />
                  </span>
                </Button>
              </div>
            </div>
        ...
        <div className="mt-16 md:mt-24">
          ...
          >
            <Tab key="specs" title="Performance Specs">
                    ...
                    {user ? (
                      hasAlreadyReviewed ? (
                        <div className="bg-white/20 p-4 rounded-xl flex flex-col items-center gap-2">
                          <ShieldAlert size={20} />
                          <span className="text-[10px] font-black uppercase">You've left your mark here!</span>
                        </div>
                      ) : !showReviewForm && (
                        <Button
                      ...
                      <p className="text-[10px] font-black uppercase bg-black/10 p-4 rounded-xl">Login to Review</p>
                    )}
                  </div>
                </div>
                      ...
                      <CardBody className="p-6">
                        <div className="flex justify-between items-center mb-6">
                          <h3 className="font-black uppercase text-sm tracking-tight">
                            {editingReview ? "Re-Tune Review" : "Leave Your Mark"}
                          </h3>
                          <button
                  ...
                  <div className="space-y-4">
                    {reviews.length === 0 ? (
                       <div className="text-center py-12 bg-gray-50 dark:bg-gray-900/50 rounded-3xl border border-dashed">
                          <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">No marks on the road yet.</p>
                       </div>
                    ) : (
                      reviews.map((r: any) => (
                        <div
