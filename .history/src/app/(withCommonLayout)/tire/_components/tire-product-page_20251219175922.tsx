    ...
    <div className="h-full overflow-y-auto pb-20">
      <div className="space-y-8 p-6">
            ...
            {widths.map((w: any) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-sm border ${
                  selectedWidths.includes(w.toString())
          ...
          <div className="flex flex-wrap gap-2">
            {ratios.map((r: any) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-sm border ${
                  selectedRatios.includes(r.toString())
                    ? "bg-orange-500 text-white"
                    : "bg-gray-100"
                }`}>
                {r}
          ...
          <div className="flex flex-wrap gap-2">
            {diameters.map((d: any) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-sm border ${
                  selectedDiameters.includes(d.toString())
                    ? "bg-orange-500 text-white"
                    : "bg-gray-100"
                }`}>
                {d}
        ...
        <Button
          ...
          className="w-full bg-orange-600 text-white mt-4">
          Reset All
        </Button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen container mx-auto px-4 py-8">
      ...
      <div className="lg:grid lg:grid-cols-[280px_1fr] lg:gap-8">
        {/* Sidebar wrapper */}
        <aside
          className={`fixed lg:relative z-50 lg:z-0 inset-y-0 left-0 w-72 bg-white transition-transform ${mobileFilterOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
          <FiltersComponent />
        </aside>
