...
const ProductCard = ({ tire }: { tire: any }) => {
  ...
  return (
    <div className="relative mt-20 mb-2 group h-full">
      ...
      <div className="bg-white dark:bg-[#1a1d23] rounded-[48px] p-5 sm:p-7 pt-24 border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col h-full transition-all duration-500 hover:shadow-2xl">
        ...
        <div className="absolute top-8 right-8 flex flex-col gap-1.5 items-end">
          <div className="bg-gray-50/80 dark:bg-[#242933] px-3 py-1 rounded-xl flex items-center gap-2 border border-gray-200/50">
            <CloudSun size={14} className="text-orange-500" />
            <span className="text-[10px] font-black uppercase text-gray-500 tracking-tight">All-Season</span>
          </div>
          <div className="bg-gray-50/80 dark:bg-[#242933] px-3 py-1 rounded-xl flex items-center gap-2 border border-gray-200/50">
            <ShieldCheck size={14} className="text-blue-600" />
            <span className="text-[10px] font-black uppercase text-gray-500 tracking-tight">85k mi Warranty</span>
          </div>
        </div>

        {/* --- ৩. প্রোডাক্ট নেম ও ব্র্যান্ড (Marketing Optimized) --- */}
        <div className="mt-1=3 mb-4 cursor-pointer" onClick={() => router.push(`/tire/${tire?._id}`)}>
          <p className="text-[11px] font-black text-blue-600 uppercase tracking-[0.25em] mb-2">{tire.brand?.name}</p> 
          <h3 className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white uppercase italic tracking-tighter leading-tight group-hover:text-blue-700 transition-colors">
            {tire?.name}
          </h3>
          <div className="flex items-center gap-1.5 mt-2">
             <Star size={12} className="fill-orange-500 text-orange-500" />
             <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Premium Performance Gear</span>
          </div>
        </div>

        {/* --- ৪. হাইলাইটেড বাল্ক ডিলস (Default Gradients & Better Text) --- */}
        {(tire.twoSetDiscountPrice || tire.fourSetDiscountPrice) && (
          <div className="grid grid-cols-2 gap-3 mb-4"> {/* Reduced bottom margin to fix your red border space */}
            {tire.twoSetDiscountPrice && (
              <button 
                onClick={() => onAddToCart(2)}
                className="relative overflow-hidden group/btn bg-gradient-to-br from-gray-800 to-gray-900 py-2.5 px-3 rounded-2xl shadow-md active:scale-95 transition-all"
              >
                <div className="flex items-center justify-between gap-1">
                  <span className="text-[9px] font-black text-white uppercase leading-none">Buy Pair (2)</span>
            ...
            {tire.fourSetDiscountPrice && (
              <button 
                ...
                className="relative overflow-hidden group/btn bg-gradient-to-br from-slate-800 to-slate-900 py-2.5 px-3 rounded-2xl shadow-md active:scale-95 transition-all"
              >
                <div className="flex items-center justify-between gap-1">
                  <span className="text-[9px] font-black text-white uppercase leading-none">Full Set (4)</span>
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-5 border-t border-gray-100 dark:border-gray-800">
          <div className="flex flex-col shrink-0">
            <span className="text-[11px] text-gray-400 line-through font-bold mb-0.5">${tire?.price}</span>
          ...
          >
            <span className="pl-6 pr-3 text-[11px] font-black uppercase tracking-widest">Buy Now</span>
