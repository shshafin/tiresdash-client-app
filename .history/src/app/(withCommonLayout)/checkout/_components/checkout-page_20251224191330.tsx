...
interface CartItem {
  ...
  installationFee?: number; // Backend Field
  addonPrice?: number; // Backend Field
}
...
const CheckoutPage = () => {
  ...
  const {
    items: cartItems = [],
    totalItems = 0,
    totalPrice = 0, // This is our source of truth from backend
    _id,
  } = data?.data || {};
  ...
  // ✅ LOGICAL FIX: No more LocalStorage or Frontend Loops
  // Directly sync the UI with the backend's totalPrice
  useEffect(() => {
    if (totalPrice > 0) {
      setCalculatedTotal(totalPrice);
    }

    // Display Savings (Original Price vs Discounted Price)
    let totalDiscount = 0;
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        <div className="grid gap-8 lg:grid-cols-5">
          ...
          {/* Order Summary - DESIGN KEPT - LOGIC FIXED */}
            ...
            <Card className="p-5 rounded sticky top-4">
              ...
              <CardBody className="grid gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  {/* ✅ FIXED: Calculation directly from backend data */}
                  {cartItems.some(
                    (item: any) =>
                      item.installationFee > 0 || item.addonPrice > 0
                  ) && (
...
// --- Stripe/Paypal Button Components logic kept exactly as your original ---
