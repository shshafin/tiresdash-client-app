...
const CheckoutPage = () => {
  ...
  // Calculate total with discounts and services
  useEffect(() => {
      ...
      cartItems.forEach((item: CartItem) => {
        ...
        if (item.productDetails) {
          if (item.quantity >= 4 && item.productDetails.fourSetDiscountPrice) {
            discountPerUnit = item.productDetails.fourSetDiscountPrice;
          } else if (item.quantity >= 2 && item.productDetails.twoSetDiscountPrice) {
            discountPerUnit = item.productDetails.twoSetDiscountPrice;
          }
        }

        totalDiscount += discountPerUnit * item.quantity;

        // Base price for the item with discount
        const itemTotal = (item.price - discountPerUnit) * item.quantity;
        total += itemTotal;
        ...
        if (
          ...
          const installCost = item.productDetails.installationPrice * item.quantity;
          total += installCost;
          servicesTotal += installCost;
        }
        ...
        if (
          ...
          selectedServices[item.product].addonServices.forEach(
            (serviceIndex: string) => {
              ...
              if (service?.price) {
                const serviceCost = service.price * item.quantity;
                total += serviceCost;
                  ...
                  {cartItems.some((item: CartItem) =>
                    ...
                    selectedServices[item.product]?.addonServices?.length > 0
                      ...
                      <div className="flex items-center justify-between text-muted-foreground">
                        ...
                        <span>+${(calculatedTotal - (cartItems.reduce((acc: number, item: CartItem) => {
                          let discount = 0;
                          if (item.productDetails) {
                            if (item.quantity >= 4 && item.productDetails.fourSetDiscountPrice) {
