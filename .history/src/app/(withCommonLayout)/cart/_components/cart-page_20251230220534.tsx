...
const CartPage = () => {
  ...
  useEffect(() => {
    // ব্যাকএন্ড থেকে আসা totalPrice কেই আমরা মেইন টোটাল হিসেবে ধরবো
    // যদি কোনো কারণে totalPrice না থাকে, তবেই কেবল ফ্রন্টএন্ড ক্যালকুলেশন কাজ করবে
    if (totalPrice) {
      setCalculatedTotal(totalPrice);
    } else {
      // ফলব্যাক ক্যালকুলেশন (যদি ব্যাকএন্ড ডাটা না থাকে)
      let total = 0;
      cartItems.forEach((item: CartItem) => {
        const baseProductTotal = item.price * item.quantity;
        const installationTotal = (item.installationFee || 0) * item.quantity;
        const addonTotal = (item.addonPrice || 0) * item.quantity;

        total += baseProductTotal + installationTotal + addonTotal;
      });
      setCalculatedTotal(total);
    }
  }, [cartItems, totalPrice]); // cartItems বা totalPrice চেঞ্জ হলেই এটা ট্রিগার হবে
