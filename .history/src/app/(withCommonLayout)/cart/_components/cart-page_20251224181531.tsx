...
const CartPage = () => {
  ...
  // ✅ Initialize selected services from BACKEND cart data
  useEffect(() => {
    if (cartItems?.length > 0) {
      const initialServices: SelectedServices = {};
      cartItems.forEach((item: CartItem) => {
        // Check if backend has stored services
        const hasInstallation = (item.installationFee || 0) > 0;
        const backendAddonServices = item.addonServices || [];

        // Map backend addon services to their indices
        const addonIndices: string[] = [];
        if (backendAddonServices.length > 0) {
          item.productDetails.addonServices?.forEach(
            (service: any, index: number) => {
              const isSelected = backendAddonServices.some(
                (addon: any) =>
                  addon._id?.toString() === service._id?.toString()
              );
              if (isSelected) {
                addonIndices.push(index.toString());
              }
            }
          );
        }
  ...
  if (cartItems?.length > 0) {
    ...
    cartItems.forEach((item: CartItem) => {
      ...
      // Check what installation data exists
      console.log("\n📦 INSTALLATION DATA FROM CART:");
      console.log("  item.installationFee:", item.installationFee);
      console.log("  item.installationService:", item.installationService);
      
      console.log("\n📦 INSTALLATION DATA FROM PRODUCT DETAILS:");
      console.log("  item.productDetails.installationPrice:", item.productDetails.installationPrice);
      console.log("  item.productDetails.installationService:", item.productDetails.installationService);
      
      // Check addon data
      console.log("\n📦 ADDON DATA FROM CART:");
      console.log("  item.addonServices:", item.addonServices);
      console.log("  item.addonPrice:", item.addonPrice);
      
      console.log("\n📦 ADDON DATA FROM PRODUCT DETAILS:");
      console.log("  item.productDetails.addonServices:", item.productDetails.addonServices);
      
      // Check selected services state
      console.log("\n✅ SELECTED SERVICES STATE:");
      console.log("  installation:", selectedServices[item.product]?.installation);
      console.log("  addonServices:", selectedServices[item.product]?.addonServices);
    });
    
    console.log("\n=================================");
    console.log("💰 TOTAL CALCULATIONS:");
    console.log("Backend totalPrice:", totalPrice);
    console.log("Frontend calculatedTotal:", calculatedTotal);
    console.log("=================================\n");
  }
}, [cartItems, selectedServices, totalPrice, calculatedTotal]);

  // ✅ Auto-sync services to backend when they change
  const syncServicesToBackend = (productId: string, productType: string) => {
    updateServices({
      productId,
      productType,
      selectedServices: {
        installation: selectedServices[productId]?.installation || false,
        addonServices:
          selectedServices[productId]?.addonServices?.map((idx: string) =>
            parseInt(idx)
          ) || [],
      },
    });
  ...
  const handleInstallationToggle = (productId: string, productType: string) => {
    setSelectedServices((prev) => {
      ...
      setTimeout(() => syncServicesToBackend(productId, productType), 100);
