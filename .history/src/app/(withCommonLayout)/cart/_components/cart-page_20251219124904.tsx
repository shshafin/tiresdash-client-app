...
useEffect(() => {
  if (cartItems?.length > 0) {
    ...
    cartItems.forEach((item: CartItem) => {
      /** * 1. BASE ITEM TOTAL 
       * We use item.price because our new backend logic already 
       * calculated the bulk discount (2set/4set) for us!
       */
      let currentItemTotal = item.price * item.quantity;

      // 2. ADD LIVE SERVICES (Installation)
      // Only add if selected on UI and NOT already included in backend price
      if (
        selectedServices[item.product]?.installation &&
        item.productDetails.installationPrice
      ) {
        // If your backend getProductDetails ALREADY adds the fee, 
        // you don't need to add it here again. 
        // But if the cart UI allows toggling it ON/OFF, keep this:
      ...
      if (
        ...
        selectedServices[item.product].addonServices.forEach((serviceIndex: string) => {
  ...
  useEffect(() => {
    if (Object.keys(selectedServices).length > 0) {
      localStorage.setItem("cart_selected_services", JSON.stringify(selectedServices));
    }
  }, [selectedServices]);
          ...
          <Card className="p-5 rounded">
            ...
            <CardBody className="grid gap-4">
              ...
              {cartItems.some((item: CartItem) => {
                return (item.quantity >= 4 && item.productDetails.fourSetDiscountPrice) ||
                  (item.quantity >= 2 && item.productDetails.twoSetDiscountPrice);
              }) && (
                  <div className="flex items-center justify-between text-green-600">
                    ...
                    <span>
                      ...
                      {cartItems.reduce((sum: number, item: CartItem) => {
                        let discount = 0;
                        if (item.quantity >= 4 && item.productDetails.fourSetDiscountPrice) {
                          discount = item.productDetails.fourSetDiscountPrice;
                        } else if (item.quantity >= 2 && item.productDetails.twoSetDiscountPrice) {
                          discount = item.productDetails.twoSetDiscountPrice;
                        }
