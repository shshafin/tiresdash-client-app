...
export default function CreateDealPage() {
  ...
  const { mutate: handleCreateDeal, isPending: createDealPending } =
    useCreateDeal({
      onSuccess: () => {
...
const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    // FIXED LOGIC: 
    // 1. Ensure it's a string before splitting (Hero UI Select usually returns a string for multiple)
    // 2. Filter out empty strings to prevent validation errors
    let reformApplicableProducts: string[] = [];
    
    if (typeof data.applicableProducts === "string") {
      reformApplicableProducts = data.applicableProducts
        .split(",")
        .map((item: string) => item.trim())
        .filter((item: string) => item !== "");
    } else if (Array.isArray(data.applicableProducts)) {
      reformApplicableProducts = data.applicableProducts;
    }
    ...
    formData.append(
      ...
      JSON.stringify({
        ...
        discountPercentage: Number(data.discountPercentage), // Ensure number type
        brand: data.brand,
