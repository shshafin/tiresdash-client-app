...
export default function UpdateWheelPage({
  ...
  const { mutate: handleUpdateWheel, isPending: updatePending } =
    useUpdateWheel({
      onSuccess: () => {
        ...
        toast.success("Wheel Data & Assets Synchronized!");
      },
      id: params.id,
    });
  ...
  const onEditSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    // ✅ লজিক ১: ইমেজ সিঙ্ক (ডিলিট করা ইমেজ বাদ দিয়ে বাকিগুলোর পাথ নাও)
    const remainingOldImages = imagePreviews
      .filter((src) => src.includes("/storage"))
      .map((src) => src.replace(envConfig.base_url as string, ""));

    // ✅ লজিক ২: নাম্বার হ্যান্ডলিং (মুছে ফেললে null যাবে যাতে ডাটাবেস আপডেট হয়)
    const toNum = (val: any) =>
    ...
    const wheelData: any = {
      ...
      images: remainingOldImages, // এইটাই ডিলিট ফিক্স করবে

      // নাম্বার ফিল্ডস (Fix for Discount & Buttons)
  ...
  return (
    <div className="p-6 bg-[#fcfcfc] dark:bg-black min-h-screen pb-24 text-gray-900 dark:text-white">
      <FormProvider {...methods}>
        <form
          ...
          <div className="bg-white dark:bg-[#0f1115] p-8 rounded-[40px] border border-gray-100 dark:border-gray-800 shadow-sm border-l-8 border-l-rose-600">
            ...
            <p className="mt-2 text-[10px] text-orange-500 font-bold uppercase tracking-widest italic">
              * Empty 2/4 set fields to disable buttons in shop
            </p>
          </div>
          ...
          {/* SECTION: TECHNICAL SPECIFICATIONS (All fields from Interface) */}
...
const MakeSelectForWheel = ({ defaultValue, register }: any) => {
  ...
  return (
    <select
      ...
      className="w-full border-2 border-[#71717ab3] bg-default-50 rounded-lg px-2 py-3.5 text-xs font-bold">
