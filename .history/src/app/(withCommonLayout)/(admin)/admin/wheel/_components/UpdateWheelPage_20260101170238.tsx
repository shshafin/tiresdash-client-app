...
export default function UpdateWheelPage({
  ...
  const onEditSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    // ✅ নাম্বার হ্যান্ডলিং (Empty string -> null)
    const toNum = (val: any) =>
    ...
    <div className="p-6 bg-[#fcfcfc] dark:bg-black min-h-screen pb-24">
      <FormProvider {...methods}>
        <form
          ...
          {/* SECTION 3: CORE SPECS (GTIN, Bolts, Offsets, etc.) */}
          ...
          <div className="bg-white dark:bg-[#0f1115] p-8 rounded-[40px] border border-gray-100 dark:border-gray-800 border-l-8 border-l-blue-600">
            <div className="flex items-center gap-3 mb-6">
          ...
          <div className="bg-white dark:bg-[#0f1115] p-8 rounded-[40px] border border-gray-100 dark:border-gray-800 shadow-sm">
            ...
            <label
              htmlFor="images"
              className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-[32px] cursor-pointer bg-gray-50 border-gray-300">
              <UploadCloud
                size={30}
                className="text-gray-400"
              />
              <span className="text-xs font-bold uppercase tracking-widest text-gray-400 mt-2">
                Update Wheel Gallery
              </span>
              <input
                multiple
                className="hidden"
