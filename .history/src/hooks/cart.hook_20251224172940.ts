...
export const useRemoveItemFromCart = ({ userId, onSuccess }: any) => {
...
export const useUpdateCartItem = ({ userId, onSuccess }: any) => {
  const queryClient = useQueryClient();
  return useMutation({
...
// ✅ NEW: Mutation: Update cart item services
export const useUpdateCartItemServices = ({ userId, onSuccess }: any) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationKey: ["UPDATE_CART_ITEM_SERVICES"],
    mutationFn: async (item: any) => await updateCartItemServices(userId, item),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["GET_CART", userId] });
      onSuccess?.();
    },
