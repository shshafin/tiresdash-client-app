...
export const removeItemFromCart = async (
  ...
  try {
    const { data } = await axiosInstance.put(
      `/cart/${userId}/items/${item?.productId}`,
...
// ✅ FIXED: Correct endpoint without userId in path
export const updateCartItemServices = async (
  ...
  try {
    const { data } = await axiosInstance.post(
      `/cart/update-services`, // ✅ Fixed: removed userId from path
      {
    ...
    console.error(error);
    throw new Error("Failed to update cart services");
  }
};
