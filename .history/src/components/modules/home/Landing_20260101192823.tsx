...
import { ChevronRight, ShieldCheck, Zap, Flame } from "lucide-react";

export default function Landing() {
  const { data, isLoading } = useGetAllDeals();
  const deals = data?.data || [];

  // ✅ ডিলস না থাকলে এই স্পোর্টি মার্কেটিং মেসেজটা দেখাবে
  const activePromo =
  ...
  return (
    <div className="pb-20 bg-white dark:bg-black">
      ...
      {/* 🏎️ Sporty Hero Section with Dark Overlay */}
      <div className="relative w-full overflow-hidden group">
        {/* Cinematic Video Background */}
        <div className="relative h-[450px] sm:h-[550px] lg:h-[680px] w-full overflow-hidden">
          {/* এগ্রেসিভ ডার্ক গ্রেডিয়েন্ট */}
          <div className="absolute inset-0 bg-black/50 z-10" />
          <div className="absolute inset-0 bg-gradient-to-t from-white dark:from-black via-transparent to-black/60 z-20" />

          <video
            className="w-full h-full object-cover scale-105 group-hover:scale-100 transition-transform duration-[15000ms] ease-out"
            autoPlay
            muted
            loop
            playsInline>
        ...
        {/* ⚡ High-Speed Hero Text */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-2/3 z-30 w-full text-center px-4 pointer-events-none">
          <h1 className="text-6xl md:text-9xl font-black text-white uppercase italic tracking-tighter drop-shadow-[0_10px_10px_rgba(0,0,0,0.5)]">
            Tires
            <span className="text-orange-600 inline-block hover:scale-110 transition-transform">
              Dash
            </span>
          </h1>
          <div className="flex items-center justify-center gap-4 mt-2">
            <div className="h-[2px] w-12 bg-orange-600" />
            <p className="text-white/90 font-black uppercase tracking-[0.5em] text-[10px] md:text-sm italic">
        ...
        {/* 🛠️ Action Area (Floating Backdrop Blur) */}
        <div className="relative z-40 -mt-28 sm:-mt-40 max-w-6xl mx-auto px-4">
          <div className="bg-white/90 dark:bg-[#0f1115]/95 backdrop-blur-3xl rounded-[40px] sm:rounded-[56px] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.5)] border border-white/20 dark:border-gray-800 p-2 sm:p-5">
            <ProductsAndServices />
          </div>
        </div>
      </div>
