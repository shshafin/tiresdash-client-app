...
const Footer = () => {
  return (
    <footer className="relative bg-[#050608] text-gray-400 pt-12 pb-6 overflow-hidden">
            ...
            <div className="flex gap-3">
              {[Facebook, Instagram, Twitter].map((Icon, idx) => (
                <a
                  key={idx}
                  href="#"
                  className="p-2 bg-white/5 rounded-lg hover:bg-orange-600 hover:text-white transition-all">
                  <Icon size={16} />
                </a>
              ))}
            </div>
              ...
              {["Terms", "Privacy", "Refund", "FAQs"].map((item) => (
                <li key={item}>
                  <Link
                    ...
                    className="text-[11px] font-bold uppercase tracking-tighter hover:text-orange-500 transition-colors">
                    {item} & Conditions
                  </Link>
                </li>
              ))}
            ...
            <ul className="grid grid-cols-1 gap-2">
              {["Deals", "Blog", "Appointments", "Contact"].map((item) => (
                <li key={item}>
                  <Link
                    href={`/${item.toLowerCase()}`}
                    className="text-[11px] font-bold uppercase tracking-tighter hover:text-orange-500 flex items-center gap-2 group transition-all">
                    <ChevronRight
                      size={10}
                      className="text-orange-600 opacity-0 group-hover:opacity-100 transition-all"
                    />
                    {item}
                  </Link>
                </li>
