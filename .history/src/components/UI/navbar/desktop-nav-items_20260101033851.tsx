...
const UniversalDropdown = ({
  ...
  return (
    <div
      className="bg-white dark:bg-[#0f1115] border border-gray-100 dark:border-gray-800 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] p-8 animate-in fade-in zoom-in-95 duration-200"
      style={{ width }}>
      ...
      <ul className="flex items-center gap-1">
        {navItems.map((item) => (
          <NavbarItem
            ...
            className="static">
            {" "}
            {/* ✅ 'static' fixes the container issue */}
            <div
              ...
              {/* ✅ Position absolute but relative to the nav container */}
              {item.hasDropdown && hoveredItem === item.label && (
                <div className="absolute left-0 right-0 top-[100%] flex justify-center pt-2 z-[9999]">
                  <div
                    className="mx-auto"
                    onMouseEnter={() => setHoveredItem(item.label)}>
